...
\newpage
# 7. Cross Validation on Neural Network
...
for(i in 1:2){
  ...
  model.nnet.10 <- nnet(stroke ~ ., data = data.train, size=10, maxit=10000, rang=0.1, decay=5e-3, MaxNWts = 20000, trace=FALSE)
  model.nnet.20 <- nnet(stroke ~ ., data = data.train, size=20, maxit=10000, rang=0.1, decay=5e-3, MaxNWts = 20000, trace=FALSE)
  
  ## 4) Prediction on test data
  model.nnet.10.pred <- predict(model.nnet.10, data.test, type="class")
  model.nnet.20.pred <- predict(model.nnet.20, data.test, type="class")
  ...
  conf_matrix <- confusionMatrix(as.factor(model.nnet.20.pred), data.test$stroke, positive="1")
  sens.nnet.20[i] <- conf_matrix$byClass["Sensitivity"]
}
```
