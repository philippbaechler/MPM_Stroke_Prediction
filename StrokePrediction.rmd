...
```{r}
set.seed(42)
d.svm <- svm(stroke ~ . - avg_glucose_level - bmi - age.int, data=d.train, kernel = "linear", type="C-classification", cost = 5)
summary(d.svm)
```
...
for(i in 1:3){
  ...
  ## 3) Train SVM
  model.svm.lin <- svm(stroke ~ . - avg_glucose_level - bmi - age.int, data=d.train, kernel = "linear", type="C-classification", cost = 5)
  model.svm.rad <- svm(stroke ~ . - avg_glucose_level - bmi - age.int, data=d.train, kernel = "radial", type="C-classification", cost = 5)
  
  ## 3) Train neuronal network
  model.nnet.10 <- nnet(stroke ~ . - avg_glucose_level - bmi - age.int, data = data.train, size=10, maxit=10000, rang=0.1, decay=5e-3, MaxNWts = 20000, trace=FALSE)
  model.nnet.15 <- nnet(stroke ~ . - avg_glucose_level - bmi - age.int, data = data.train, size=15, maxit=10000, rang=0.1, decay=5e-3, MaxNWts = 20000, trace=FALSE)
...
```{r}
boxplot(sens.svm.lin, sens.svm.rad, sens.nnet.10, sens.nnet.15, names=c("svm.lin", "svm.rad", "nnet.10", "nnet.15"))
```
